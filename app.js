...
const config = require('./cli.js')
	...
	.option(['-d', '--httpdns2'], {action: 'httpdns2', help: 'enable httpdns2'})
...
if (config.httpdns2) {
	console.log(`HttpDns2 State : [Enable]`);
	...
	httpArray = [httpdns, httpdns2];
} else {
	console.log(`HttpDns2 State : [Disable]`);
	httpArray = [httpdns];
}

Promise.all(httpArray.map(query => query(hook.target.host.join(','))).concat(hook.target.host.map(host => dns(host))))
.then(result => {
	...
	if(port[0]){
		...
		console.log(`HTTP Server State : [Running] @ http://${address || '0.0.0.0'}:${port[0]}`)
	}
	if(port[1]){
		server.https.listen(port[1], address);
		console.log(`HTTPs Server State : [Running] @ http://${address || '0.0.0.0'}:${port[1]}`)
	}
	console.log(``);
	console.log(`Pass Ctrl+C to Stop Server`);
})
